...
//    }
    ...
    class func loadObjectsWithName(objectName: String , inNode: SKNode , intoLayer: SKNode){
        
        if let layer_node = inNode.childNodeWithName(objectName) {
            layer_node.enumerateChildNodesWithName("*") { node , stop in
                ...
                else if(node_sprite.name!.hasPrefix("spike_")){
                    node_sprite.physicsBody!.categoryBitMask = ColliderType.Spike.rawValue
                    node_sprite.physicsBody!.affectedByGravity = false
                    node_sprite.physicsBody!.dynamic = false
                }
            }
